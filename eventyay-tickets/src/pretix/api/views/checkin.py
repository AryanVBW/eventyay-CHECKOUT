...
class BulkCheckoutView(views.APIView):
    """
    API view for performing bulk checkout of all attendees in an event.
    """
    
    def post(self, request, *args, **kwargs):
        from pretix.base.services.checkout import perform_bulk_checkout, CheckOutError
        ...
        event = get_object_or_404(events, slug=self.kwargs['event'])
        
        # Parse request data
        checkin_list_ids = request.data.get('checkin_lists', [])
        force = request.data.get('force', False)
        datetime_str = request.data.get('datetime')
        
        # Validate checkin lists
        checkin_lists = None
        if checkin_list_ids:
            checkin_lists = list(event.checkin_lists.filter(id__in=checkin_list_ids))
            if len(checkin_lists) != len(checkin_list_ids):
                missing_lists = set(checkin_list_ids) - {cl.id for cl in checkin_lists}
                return Response(
                    {'error': f'Invalid checkin list IDs: {", ".join(map(str, missing_lists))}'},
                    status=400
                )
        ...
        if datetime_str:
            try:
                checkout_datetime = DateTimeField().to_internal_value(datetime_str)
            except Exception:
                return Response({'error': 'Invalid datetime format'}, status=400)
        
        try:
            # Perform bulk checkout
            results = perform_bulk_checkout(
                event=event,
            ...
            return Response({
                'status': 'success',
                'results': results,
                'message': f'Bulk checkout completed. {results["success_count"]} attendees checked out, {results["error_count"]} errors.'
            })
            
        except Exception as e:
            return Response(
                {'error': f'Bulk checkout failed: {str(e)}'},
                status=500
            )


class SingleCheckoutView(views.APIView):
    """
    API view for performing checkout of a single attendee.
    """
    
    def post(self, request, *args, **kwargs):
        from pretix.base.services.checkout import perform_single_checkout, CheckOutError
        ...
        checkin_list_id = request.data.get('checkin_list_id')
        force = request.data.get('force', False)
        datetime_str = request.data.get('datetime')
        
        if not position_id or not checkin_list_id:
            return Response(
                {'error': 'position_id and checkin_list_id are required'},
                status=400
            )
        
        # Get position and checkin list
        try:
            position = OrderPosition.objects.get(
                id=position_id,
                order__event=event
            )
            checkin_list = event.checkin_lists.get(id=checkin_list_id)
        except (OrderPosition.DoesNotExist, CheckinList.DoesNotExist):
            return Response({'error': 'Invalid position or checkin list'}, status=404)
        ...
        try:
            ...
            checkout = perform_single_checkout(
                position=position,
