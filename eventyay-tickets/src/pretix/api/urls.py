...
urlpatterns = [
    ...
    url(
        r'^organizers/(?P<organizer>[^/]+)/events/(?P<event>[^/]+)/checkout/bulk/$',
        checkin.BulkCheckoutView.as_view(),
        name='checkout.bulk',
    ),
    url(
        r'^organizers/(?P<organizer>[^/]+)/events/(?P<event>[^/]+)/checkout/single/$',
        checkin.SingleCheckoutView.as_view(),
